        ...
        <div className="flex items-center gap-0.5">
            {symbols.slice(0, 6).map((s, i) => {
                ...
                return (
                    <span
                        key={i}
                        className={`inline-flex items-center justify-center w-4 h-4 rounded-full text-[9px] font-bold border border-black/20 ${
...
function CardRow({ card, onVariantOpen, deckId, onTransferCard, onGifAlterOpen, onSuggestionsOpen, section = "main" }: CardRowProps) {
    ...
    return (
        <div className="group flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-secondary/50 transition-colors animate-fade-in-up">
            {/* Thumbnail */}
            <div className="relative w-8 h-11 rounded shrink-0 overflow-hidden bg-secondary border border-border">
                {thumbnailUrl && !imgError ? (
                    ...
                    <div className="w-full h-full bg-muted flex items-center justify-center">
                        <span className="text-[8px] text-muted-foreground text-center leading-tight px-0.5">{card.name.slice(0, 4)}</span>
                    </div>
                )}
                {card.isCommander && (
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                    <span
                        className={`text-sm font-medium truncate leading-none ${isBanned ? "text-red-400 line-through" : "text-foreground"}`}
                    >
                        {card.name}
                    </span>
                    ...
                    {isBanned && (
                        <Badge
                            variant="outline"
                            className="text-[8px] px-1 py-0 border-red-500 text-red-400 bg-red-500/5 uppercase font-mono animate-pulse"
                        >
                            Banned
                        </Badge>
                    )}
                    {isCover && (
                        <Badge
                            variant="outline"
                            className="text-[8px] px-1 py-0 border-primary text-primary bg-primary/5 uppercase font-mono"
                        >
                            Cover
                        </Badge>
                    )}
            ...
            <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                {section === "main" && (
                    <Button
                        ...
                        className={`w-6 h-6 transition-colors ${
                            isCover ? "text-primary bg-primary/10" : "text-muted-foreground hover:text-primary"
                        }`}
                ...
                {section !== "tokens" && (
                    <>
                        ...
                        <span className="w-6 text-center text-sm font-mono font-semibold text-foreground">{card.quantity}</span>
    ...
    return (
        <div className="group flex items-center justify-between py-1 px-1.5 rounded-md hover:bg-secondary/40 transition-colors text-xs select-none">
            <div className="flex items-center gap-1.5 min-w-0 flex-1">
                {/* Quantity */}
                {section !== "tokens" && (
                    <span className="font-mono font-bold text-neutral-400 w-5 shrink-0 text-left">{card.quantity}</span>
                )}

                {/* Card Name with Badges */}
                <div className="flex items-center gap-1 min-w-0 truncate">
                    <span
                        className={`font-medium truncate hover:text-primary transition-colors cursor-pointer ${isBanned ? "text-red-400 line-through" : "text-foreground"}`}
                        title={card.name}
                        onClick={() => onVariantOpen(card, section)}
                    >
                        {card.name}
                    ...
                    {isBanned && (
                        <span
                            className="text-[8px] font-bold text-red-500 bg-red-500/10 px-1 py-0.5 rounded shadow-sm border border-red-500/15 shrink-0 animate-pulse"
                    ...
                    {isGc && (
                        <span
                            className="text-[8px] font-bold text-amber-500 bg-amber-500/10 px-1 py-0.5 rounded shadow-sm border border-amber-500/15 shrink-0 animate-pulse"
                            title="Game Changer Card"
                        >
                            GC
                        </span>
                    )}
                    {hasAlter && (
                        <span
                            className="text-[8px] font-bold text-blue-400 bg-blue-500/10 px-1 py-0.5 rounded shadow-sm border border-blue-500/15 shrink-0"
                            title="Custom Alter Art"
                        >
                            CB
                        </span>
                    )}
            ...
            <div className="space-y-2.5">
                {cardData.card_faces.map((face: any, idx: number) => (
                    <div key={idx} className="text-left border-l-2 border-primary/40 pl-2 py-0.5">
                        ...
                        {face.type_line && (
                            <div className="text-[8px] text-muted-foreground font-semibold italic truncate mb-1">{face.type_line}</div>
                        )}
                        {face.oracle_text &&
                ...
                <>
                    ...
                    {viewMode === "grid" && (
                        <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] sm:grid-cols-[repeat(auto-fill,minmax(250px,1fr))] gap-2 px-2 py-2 justify-items-center">
                            {cards.map((card) => {
                                const cardImg =
                                    card.scryfallData.image_uris?.normal || card.scryfallData.card_faces?.[0]?.image_uris?.normal || "";
                                const isCover = state?.coverCardId === card.scryfallId;
                                const isBanned = card.scryfallData.legalities?.commander === "banned";
                                return (
                                    <div
                                        key={card.scryfallId}
                                        className={`relative w-[200px] sm:w-[250px] aspect-[5/7] rounded-xl overflow-hidden border shadow-lg bg-secondary group/visual transition-all duration-300 hover:scale-[1.04] hover:shadow-2xl animate-fade-in-up ${
                                            isCover ? "border-primary ring-2 ring-primary/40" : "border-border/80"
                                        }`}
                                    >
                                        {cardImg ? (
                                            <CardMedia
                                            ...
                                            <div className="w-full h-full bg-muted p-3 flex flex-col justify-between items-center text-center">
                                                <span className="text-[11px] font-bold text-foreground truncate w-full">{card.name}</span>
                                        ...
                                        {section !== "tokens" && (
                                            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-black/85 px-3 py-1 rounded-full border border-border/40 z-10 flex items-center gap-1.5 shadow-md shadow-black/50 transition-opacity duration-300 group-hover/visual:opacity-0 select-none animate-fade-in">
                                                <span className="text-[9px] font-bold text-muted-foreground tracking-wider uppercase font-mono">
                                                    QTY:
                                                </span>
                                            ...
                                            <div className="flex flex-col gap-2 w-full">
                                                <div className="flex gap-1.5 justify-center flex-wrap">
                                                    ...
                                                    {section === "main" && (
                                                        <button
                                                            ...
                                                            className={`p-2 rounded-lg transition-all duration-200 border shadow-sm active:scale-95 ${
                                                                isCover
                                                ...
                                                {section !== "tokens" && (
                                                    <div className="flex items-center justify-between bg-black/75 border border-border/40 rounded-lg py-1 px-3 shadow-inner">
                                                        ...
                                                        <span className="text-xs font-mono font-bold text-foreground">{card.quantity}</span>
                                                        <button
                                                            onClick={() =>
                                                                dispatch({
...
export function CardList({ deckId, onTransferCard }: CardListProps = {}) {
        ...
        const timeoutId = setTimeout(() => {
            fetch(url, { signal: controller.signal })
                ...
                .then((data) => {
                    ...
                    const exactMatches = results.filter((print) => print.name.toLowerCase() === variantCard.name.toLowerCase());
    ...
    const combinedAllCards = [...state.cards, ...(state.sidedeck || []), ...(state.tokens || [])];

    const allSubtypes = Array.from(
        new Set(
            combinedAllCards.flatMap((card) => {
                const typeLine = card.scryfallData.type_line || "";
                const typeLines = card.scryfallData.card_faces ? card.scryfallData.card_faces.map((f) => f.type_line || "") : [typeLine];
    ...
    const filterCardItem = (card: DeckCard) => {
        ...
        const oracleText = [
            card.scryfallData.oracle_text || "",
            ...(card.scryfallData.card_faces?.map((face) => face.oracle_text || "") ?? []),
        ]
            .join(" ")
            .toLowerCase();
        const matchesName = card.name.toLowerCase().includes(query);
        const matchesRules = oracleText.includes(query);
    ...
    const sortedFilteredCards = sortCardList(filteredCards);
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="flex items-center gap-2 w-full sm:max-w-xs">
                        ...
                        >
                            ...
                            {(selectedCmcs.length > 0 ||
                                ...
                                selectedSubtype !== "all" ||
                                showOnlyGameChangers ||
                                showOnlyLegendary) && <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse shrink-0" />}
                        </Button>
                    </div>
                ...
                ${
                    state.isSideDeckEnabled
                        ...
                        >
                            ...
                            <span>Side Deck</span>
                            <div
                                className={`w-7.5 h-4 rounded-full relative transition-colors duration-200 p-0.5 shrink-0 flex items-center ${state.isSideDeckEnabled ? "bg-amber-500" : "bg-neutral-600"}`}
                            >
                                <div
                                    className={`w-3 h-3 rounded-full bg-white transition-transform duration-200 ${state.isSideDeckEnabled ? "translate-x-3.5" : "translate-x-0"}`}
                                />
                            </div>
                        </button>
                        ...
                        <div className="flex bg-secondary p-0.5 rounded-lg border border-border/60 shrink-0">
                            <button
                                ...
                                className={`p-1 px-2.5 rounded-md text-[10px] font-bold transition-all flex items-center gap-1.5 ${
                                    viewMode === "grid"
                        ...
                        <Button
                            ...
                            className={`h-8 text-xs gap-1.5 shrink-0 px-2.5 border-border hover:border-primary/50 hover:text-primary transition-colors ${
                                showUsdPrices ? "bg-primary/10 border-primary/30 text-primary hover:bg-primary/15" : "hover:bg-secondary"
                            }`}
                            title="Toggle USD Price"
                        >
                ...
                {showAdvancedFilters && (
                    <div className="flex flex-col gap-2 border-t border-border/20 pt-2 bg-background/5 p-2 rounded-lg border border-border/30 animate-fade-in-up">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider min-w-[75px]">
                                Mana Value:
                            </span>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <div className="flex flex-wrap gap-0.5 bg-secondary/30 p-0.5 rounded-md border border-border/40">
                                {["all", "Creature", "Planeswalker", "Instant", "Sorcery", "Artifact", "Enchantment", "Land", "Other"].map(
                                    (typeVal) => {
                            ...
                            {state.isSideDeckEnabled && sortedSidedeck.length > 0 && (
                                <div className="border-t border-border/30 pt-6">
                                    <PremiumListSection
                                        ...
                                        onVariantOpen={(card, sect) => {
                                            setVariantCard(card);
                                            setVariantCardSection(sect || "side");
                                        }}
                                        deckId={deckId}
                                        section="side"
                                        onTransferCard={onTransferCard}
                            ...
                            {sortedTokens.length > 0 && (
                                <div className="border-t border-border/30 pt-6">
                                    <PremiumListSection
                                        ...
                                        onVariantOpen={(card, sect) => {
                                            setVariantCard(card);
                                            setVariantCardSection(sect || "tokens");
                                        }}
                                        deckId={deckId}
                                        section="tokens"
                                        onTransferCard={onTransferCard}
                    ...
                    <DialogHeader>
                        ...
                        <DialogDescription className="flex flex-col sm:flex-row sm:items-start md:items-center justify-between gap-3 text-muted-foreground mt-1">
                            <span className="max-w-xl flex items-center gap-2 flex-wrap">
                                Choose your preferred printing for <strong className="text-foreground">{variantCard?.name}</strong>. This
                                will update the card art across the builder and exported files.
                        ...
                        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-6">
                                    ...
                                    return (
                                        <div className="space-y-3 bg-secondary/10 p-3.5 rounded-xl border border-border/30 animate-fade-in-up">
                                                        ...
                                                        >
                                                            ...
                                                            <button
                                                                onClick={(e) => {
                                                                    ...
                                                                    if (
                                                                        confirm(
                                                                            `Are you sure you want to delete the custom alter "${custom.name}"?`,
                                                                        )
                                                                    ) {
                                                            ...
                                                            {(() => {
                                                                const normName =
                                                                    variantCard?.name.toLowerCase().trim().split("//")[0].trim() ?? "";
                                            ...
                                            <div className="flex items-center gap-2 pt-2 text-[10px] text-muted-foreground">
                                                ...
                                                <span>
                                                    These are your local custom designs. Official printings from Scryfall are listed below.
                                                </span>
                                            </div>
                            ...
                            {loadingPrints ? (
                                <div className="flex flex-col items-center justify-center py-20 gap-3">
                                    ...
                                    <p className="text-sm text-muted-foreground animate-pulse">Fetching printings from Scryfall...</p>
                                </div>
                            ) : printsError ? (
                                ...
                                <div className="grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] sm:grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4 justify-items-center">
                                    {printings.map((print) => {
                                        const printImg = print.image_uris?.normal || print.card_faces?.[0]?.image_uris?.normal || "";
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {(() => {
                                                    const normName = variantCard?.name.toLowerCase().trim().split("//")[0].trim() ?? "";
                                                    const isFav = favoriteArts.some(
                                                        (f) => f.cardName === normName && f.scryfallId === print.id,
                                                    );
                                                    return (
                                                        <button
                                                            onClick={(e) => {
                            ...
                            {previewCard ? (
                                <div className="flex flex-col items-center text-center space-y-4 w-full h-full justify-center">
                                    ...
                                    <div className="space-y-1.5 w-full">
                                        ...
                                        {previewCard.artist && (
                                            <p className="text-[10.5px] text-primary/80 italic font-semibold truncate mt-0.5">
                                                Illustrated by {previewCard.artist}
                                            </p>
                                        )}
            ...
            <CustomCardsModal
                ...
                prepopulatedCard={variantCard ? { id: variantCard.scryfallId, name: variantCard.name } : null}
            />

            {/* Commander GIF Alter Modal */}
            {gifAlterCard && (
                <CommanderGifAlterModal card={gifAlterCard} open={gifAlterCard !== null} onClose={() => setGifAlterCard(null)} />
            )}
