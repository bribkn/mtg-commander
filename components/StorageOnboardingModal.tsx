      ...
      <DialogContent className="w-full sm:max-w-2xl bg-card border-border shadow-2xl shadow-black/60 [&>button]:hidden p-0 overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-6">
          ...
          <button
            ...
            className={`relative flex flex-col items-start gap-4 p-5 rounded-2xl border-2 transition-all duration-200 text-left cursor-pointer disabled:cursor-not-allowed ${hovered === 'folder'
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center border transition-colors ${hovered === 'folder' ? 'bg-primary/20 border-primary/40 text-primary' : 'bg-secondary border-border text-muted-foreground'
                }`}>
            ...
            <ul className="space-y-1.5 w-full">
              {[
                ...
                { icon: <Shield className="w-3.5 h-3.5 text-emerald-400 shrink-0" />, text: 'No se borran al limpiar el browser' },
                { icon: <Zap className="w-3.5 h-3.5 text-amber-400 shrink-0" />, text: 'Compatible con Tabletop Simulator' },
              ].map(({ icon, text }) => (
          ...
          <button
            ...
            onMouseLeave={() => setHovered(null)}
            className={`relative flex flex-col items-start gap-4 p-5 rounded-2xl border-2 transition-all duration-200 text-left cursor-pointer disabled:cursor-not-allowed ${hovered === 'local'
          ...
          >
            <div className="flex items-center gap-3">
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center border transition-colors ${hovered === 'local' ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-400' : 'bg-secondary border-border text-muted-foreground'
                }`}>
            ...
            <ul className="space-y-1.5 w-full">
              {[
                { icon: <Zap className="w-3.5 h-3.5 text-amber-400 shrink-0" />, text: 'Sin configuración, listo al instante' },
                { icon: <Wifi className="w-3.5 h-3.5 text-sky-400 shrink-0" />, text: 'Funciona en cualquier dispositivo' },
                { icon: <Shield className="w-3.5 h-3.5 text-yellow-500 shrink-0" />, text: 'Se borra al limpiar datos del sitio' },
              ].map(({ icon, text }) => (
                <li key={text} className="flex items-center gap-2 text-xs text-muted-foreground">
                  {icon}<span>{text}</span>
                </li>
