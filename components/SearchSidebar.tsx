...
export function SearchSidebar({ mode, onModeChange, deckId }: SearchSidebarProps) {
    ...
    const estimatedDeckPrice = state?.cards
    ...
    const buildScryfallQuery = () => {
        ...
        const hasLandSearch = activeTypes.includes("other") || activeTypes.includes("land");
        if (!hasLandSearch) {
            parts.push("-t:land");
        }
                        ...
                        <div className="space-y-6 p-4">
                                ...
                                <div className="border border-red-500/30 bg-red-500/10 rounded-xl p-3 flex flex-col gap-1.5 shadow-lg shadow-red-500/5 animate-pulse">
                                    ...
                                    <p className="text-[10px] text-muted-foreground leading-normal">
                                        ...
                                        <strong className="text-red-400 font-bold">
                                            {bannedCount} banned card{bannedCount > 1 ? "s" : ""}
                                        </strong>{" "}
                                        in Commander:
                                    </p>
                                    <div className="flex flex-col gap-0.5 mt-1 border-t border-red-500/15 pt-1.5 max-h-24 overflow-y-auto custom-scrollbar">
                                        {bannedCardsList.map((bc) => (
                                            <div key={bc.name} className="flex justify-between gap-4 text-[10px]">
                                                <span className="font-semibold text-red-400 truncate max-w-[140px]" title={bc.name}>
                                                    {bc.name}
                                                </span>
                            ...
                            <div className="grid grid-cols-2 gap-2.5">
                                ...
                                <StatBox
                                    ...
                                    sublabel="in deck"
                                    highlight={landCount >= 34 && landCount <= 40}
                                />
                                <StatBox
                                    label="Opening Lands"
                                    value={avgOpeningLands}
                                    sublabel="avg in 7 cards"
                                    highlight={
                                        !isNaN(parseFloat(avgOpeningLands)) &&
                                        parseFloat(avgOpeningLands) >= 2.3 &&
                                        parseFloat(avgOpeningLands) <= 3.0
                                    }
                                />
                                ...
                                <StatBox
                                    label="Est. Price"
                                    value={`$${estimatedDeckPrice.toFixed(2)}`}
                                    sublabel="usd value"
                                    highlight={estimatedDeckPrice > 0}
                                />
                                ...
                                <div className="relative group/salt select-none">
                                    <div className="rounded-lg p-3 text-center border bg-secondary border-border hover:border-border/80 transition-all duration-300 cursor-help">
                                        <div className="text-xl font-bold font-mono text-orange-400">{saltSum.toFixed(2)}</div>
                                    ...
                                    <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2.5 hidden group-hover/salt:block z-50 bg-popover border border-border p-4 rounded-xl shadow-2xl w-[280px] animate-in fade-in slide-in-from-bottom-2 duration-200">
                                        ...
                                        <div className="mt-3.5 grid grid-cols-2 gap-2.5 text-[10px] bg-secondary/30 rounded-lg p-2 font-mono">
                                            ...
                                            <div className="text-center">
                                                <div className="text-muted-foreground text-[8px] uppercase font-bold mb-0.5">
                                                    Global Avg
                                                </div>
                ...
                <div className="flex flex-col h-full overflow-hidden animate-fade-in-right">
                    ...
                    <div className="p-3.5 border-b border-border bg-secondary/10 flex flex-col gap-2 shrink-0">
                        <div className="flex items-center justify-between">
                            ...
                            <Badge
                                variant="outline"
                                className="text-[10px] font-mono border-primary/30 text-primary bg-primary/5 px-2 py-0.5 select-none uppercase"
                            >
                                ...
                                <div className="p-3 border-t border-border/40 space-y-4 animate-fade-in-up">
                                    ...
                                    <div className="grid grid-cols-2 gap-1.5">
                                        {[
                                            ...
                                            <label
                                                key={item.label}
                                                className="flex items-center gap-1.5 text-[9px] font-semibold text-muted-foreground cursor-pointer hover:text-foreground select-none"
                                            >
                                                <input
                                                    type="checkbox"
                                                    checked={item.state}
